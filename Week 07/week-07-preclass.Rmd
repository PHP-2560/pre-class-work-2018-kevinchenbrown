...
max <- 5000
for (i in 1:max){
  next_duration <- gamblers_ruin(100, 900, p = 0.5)
...
prob_500
```
The probability I have busted by the time I have placed my 500th best is 0.4774.
    c. the mean time you go bust, given that you go bust within the first 5000 hands.
```{r}
mean(duration)
```
The mean time I go bust is after 761.555 hands.
...
for(i in 1:5000)
{
  next_bankroll <- gamblers_ruin_100(100, 900, 0.5)
...
for (j in 1:20) {
  ...
  M <- 100
  for(i in 1:M) {
    next_size <- markov(100, 900, 0.46+0.002*j)
    size <- c(size, next_size)
  }
  next_prop <- length(which(size == 0))/100
  prop <- c(prop,next_prop)
}
prop
```
...
for (j in 1:20) {
  ...
  M <- 100
  for(i in 1:M) {
    next_size <- markov_inc(100, 900, 0.48, 0.002*j)
    size <- c(size, next_size)
  }
  next_prop <- length(which(size == 0))/100
  prop <- c(prop,next_prop)
